...
def get_data(dataset, data_path,labels_path, cutout_length, validation,validation2 = False,n_class = 3,image_size = 64):
    ...
    if dataset == 'custom':
        ...
        for x_path in X_train:
            x = cv2.imread(data_path+x_path,0) 
        ...
        x_train_data = torch.from_numpy(x_train_data).double()
        
        x_train_data = x_train_data.permute(0, 3, 1, 2)
        print(x_train_data.shape)

        x_test_data = np.asarray(x_test_data)
        x_test_data = torch.from_numpy(x_test_data).double()
        ...
        tensor_train_x = torch.stack([torch.Tensor(i.type('torch.FloatTensor')) for i in x_train_data]) # transform to torch tensors
        tensor_test_x = torch.stack([torch.Tensor(i.type('torch.FloatTensor')) for i in x_test_data]) # transform to torch tensors
